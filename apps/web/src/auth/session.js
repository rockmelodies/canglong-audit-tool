...
const accessToken = ref(typeof window === 'undefined' ? null : window.localStorage.getItem(TOKEN_KEY));
const user = ref(typeof window === 'undefined' ? null : JSON.parse(window.localStorage.getItem(USER_KEY) ?? 'null'));
function persist() {
...
export async function restoreSession(locale) {
    if (!accessToken.value) {
        return;
    }
