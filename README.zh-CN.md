...
当前方向不是把产品做成“模型列表展示器”，而是遵循这条主线：

- 大多数 agent 任务默认先用能力最强的通用前沿模型
- 再通过提示词包、MCP、工具链和可复用 Skills 叠加专用能力
- 只有在长上下文复核、多模态制品理解或私有部署明显更优时才切换模型

| 模型通道 | 最适合场景 | 示例任务 |
| --- | --- | --- |
| GPT-5.4 类模型 | 默认审计编排、工具调用、结构化推理 | 利用链扩展、工作流执行、报告综合 |
| Opus 类长上下文模型 | 大型审阅和矛盾处理 | 守卫校验、策略复核、仓库二次审查 |
| Gemini 类多模态模型 | 截图、反编译制品、流程图、二进制侧证据 | 制品理解、逆向辅助、接口恢复 |
| 私有化通用模型 | 受隐私和合规约束的执行 | 内网部署、敏感代码审计、隔离环境运行 |
...
1. 从 `/login` 登录控制台。
2. 先打开 `/settings` 配置一个默认模型，支持自定义 OpenAI 兼容接口地址。
3. 再进入 `/workspace` 接入远程 Git 仓库或本地代码目录，并按需完成同步。
4. 在仓库卡片上点击绿色“开始审计”按钮发起任务。
5. 打开生成后的报告，查看环境指纹、依赖证据、接口发现、利用链候选、误报抑制、Docker 验证线索和修复建议。

## 当前实现快照

当前仓库已经具备一条可演示的端到端流程：

- 面向 Web UI 和 API 的会话式演示鉴权
- 同时支持远程 Git 地址和本地目录的仓库接入
- Git 源的 clone / pull 同步，以及本地目录的路径校验
- 带进度、阶段状态和报告生成的后台审计任务
- 基于启发式规则的审计报告，包含环境指纹、依赖证据、接口发现、发现项提升、利用链候选、误报抑制和 Docker 验证规划
- 登录、总览、工作台、任务和报告页的中英文双语界面
...
| `GET` | `/api/settings/models` | 获取模型配置、就绪状态和操作指引 |
| `POST` | `/api/settings/models` | 添加自定义模型接入点 |
| `PUT` | `/api/settings/models/{model_id}` | 更新模型配置 |
| `POST` | `/api/settings/models/{model_id}/default` | 设为默认审计模型 |
| `GET` | `/api/repos` | 获取已登记仓库列表 |
| `POST` | `/api/repos` | 新增 Git 仓库或本地仓库 |
| `POST` | `/api/repos/{repo_id}/sync` | clone / pull Git 仓库，或校验本地目录 |
| `GET` | `/api/audits` | 获取审计任务列表 |
| `POST` | `/api/audits` | 发起新的审计任务 |
| `GET` | `/api/audits/{job_id}` | 查询审计任务状态和阶段进度 |
| `GET` | `/api/audits/{job_id}/report` | 获取已完成的审计报告 |
...
- 任务与仓库的持久化存储
- 在当前启发式引擎之上继续深化语言感知审计
- 将 Docker 验证从规划推进到真正执行
- 反编译适配器与制品接入流程
- 真实模型 SDK 接入与私有化安全路由
- 证据导出与协作能力
